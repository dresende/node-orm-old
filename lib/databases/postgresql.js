var pg = require("/usr/local/lib/node/pg/lib/");
...
exports.connect = function (options, callback) {
	...
	pg.connect("pg://"+opts.user+":"+opts.password+"@"+opts.host+":"+opts.port+"/"+opts.database, function (err, cli) {
		if (err) {
			callback(false, { "number": parseInt(err.code), "message": err.message });
		} else {
			callback(true, new DBClient(cli));
		}
	});
};
