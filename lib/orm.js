...
ORM.prototype.define = function (model, fields, colParams) {
	...
	var addOneAssociationMethods = function (self, association, associationModel) {
		var camelCaseAssociation = association.substr(0, 1).toUpperCase() + association.substr(1);
		var collection = model;

		self.prototype["get" + camelCaseAssociation] = function () {
			var self = this, cb = null, opts = {};
		...
		self.prototype["unset" + camelCaseAssociation] = function (cb) {
			this["set" + camelCaseAssociation](null, cb);
		};
		self.prototype["set" + camelCaseAssociation] = function (instance, cb) {
			var self = this;
		...
		for (var i = 0; i < associations.length; i++) {
			switch (associations[i].type) {
				case "one":
					...
					if (this[associations[i].field + "_id"] > 0 && associations[i].opts
					&& associations[i].opts.hasOwnProperty("autoFetch") && associations[i].opts.autoFetch === true
					&& (!opts.hasOwnProperty("fetchDepth") || opts.fetchDepth > 0)) {
						var camelCaseAssociation = associations[i].field.substr(0, 1).toUpperCase()
						                         + associations[i].field.substr(1),
						    assocOpts = {};
						...
						this._dataPending += 1;
						this[associations[i].field] = null;
						this["get" + camelCaseAssociation](assocOpts, (function (m, assoc) {
							return function (result) {
								m[assoc.field] = result;
					...
					break;
				case "many":
					if (associations[i].opts && associations[i].opts.hasOwnProperty("autoFetch")
					&& associations[i].opts.autoFetch === true
					&& (!opts.hasOwnProperty("fetchDepth") || opts.fetchDepth > 0)) {
						var camelCaseAssociation = associations[i].field.substr(0, 1).toUpperCase()
						                         + associations[i].field.substr(1),
						    assocOpts = {};
